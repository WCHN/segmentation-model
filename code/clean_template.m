...
% Run clean-up of GM, WM and CSF
template = clean_gwc(template,ix,2);
...
% Run clean-up of bone and soft tissue
% template = clean_bone_st(template);

if nargout > 0
    % Take log and write to template NIfTI
    model.template.nii.dat(:,:,:,:) = log(max(template,eps('single')));
end

%==========================================================================
function template = clean_bone_st(template)

ix_st = 6;
FigNum      = 666;

% Get soft tissue class
st = template(:,:,:,ix_st);

figure(668)
imagesc3d(st); axis off; drawnow
...
figure(668)
imagesc3d(C); axis off; drawnow

% Close soft tissue class

% Invert closed soft tissue class
