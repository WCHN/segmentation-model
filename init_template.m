function model = init_template(dat,opt)
% FORMAT model = init_template(dat,opt)
...
if isfield(opt.template,'pth_template') && exist(opt.template.pth_template,'file')
    f                  = opt.template.pth_template;
    [~,nam,ext]        = fileparts(f);
    nf                 = fullfile(opt.dir_model,[nam ext]);
    copyfile(f,nf)
    model.template.nii = nifti(nf);    
else
